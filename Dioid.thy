...
where "canonic_order G  \<equiv> factor G "
...
where "dioid_canonic_order R   \<equiv> factor (add_monoid R) "

definition
  dioid_canonic_strict_order :: "('a, 'b) ring_scheme => 'a => 'a \<Rightarrow> bool" (infixl "<\<index>" 50)
where "dioid_canonic_strict_order R x y  = ((dioid_canonic_order R x y) \<and> \<not>(x = y))"
...
proof
  ...
  hence ainfone : "a \<preceq> \<one>" unfolding factor_def by force
  moreover have "\<one> \<otimes> a = a" using l_one aG by auto
  hence "\<one> \<preceq> a" unfolding factor_def using aG one_closed by auto
  ultimately show "a = \<one>" using partial_order.le_antisym[OF canonically_ordered,of a \<one>] aG by auto
  from aG bG abone have "b \<preceq> \<one>" unfolding factor_def using m_comm by force
  moreover have "\<one> \<otimes> b = b" using l_one bG by auto
  hence "\<one> \<preceq> b" unfolding factor_def using bG one_closed by auto
...
proof
  ...
  proof (rule ccontr)
    ...
    moreover have "\<one> \<preceq> x"
      unfolding factor_def using l_one x_def by force
...
proof (intro canonical_order_monoid.intro)
  ...
  proof-
    have "\<And> x . x \<in> H \<Longrightarrow> \<one> \<in> H \<and> x \<otimes> \<one> = x"
      using submonoid.one_closed[OF assms(1)]
      by (meson assms r_one submonoid.mem_carrier)
    thus "\<And>x. x \<in> H \<Longrightarrow> \<exists> z \<in> H . x = x \<otimes> z " by force
    show "\<And>x c ca. c \<in> H \<Longrightarrow> ca \<in> H \<Longrightarrow> x = x \<otimes> c \<otimes> ca \<Longrightarrow> x \<in> H \<Longrightarrow> x \<otimes> c \<in> H \<Longrightarrow> x = x \<otimes> c"
      using partial_order.le_antisym[OF canonically_ordered] assms factor_def
      by (smt  gorder.select_convs(1) partial_object.select_convs(1) submonoid.mem_carrier)
    show "\<And>x c ca. c \<in> H \<Longrightarrow> ca \<in> H \<Longrightarrow> x \<in> H \<Longrightarrow> x \<otimes> c \<in> H \<Longrightarrow> x \<otimes> c \<otimes> ca \<in> H \<Longrightarrow> \<exists>cb\<in>H.
          x \<otimes> c \<otimes> ca = x \<otimes> cb"
...
proof -
  from assms obtain x where x_def : "x \<in> carrier D" " a \<oplus> x = b" by auto
  ...
  thus "(a \<otimes> c) \<le> (b \<otimes> c)" using assms(3) x_def(1) by auto
qed

lemma (in dioid) zero_is_idem :
"\<zero> \<otimes> \<zero> = \<zero>"
proof-
  have "\<zero> \<le> \<zero> \<otimes> \<zero>" by auto
  moreover have "\<zero> \<otimes> \<zero> \<le> \<zero> \<otimes> \<zero> \<otimes> \<zero>" by auto
  ...
  proof-
    ...
    hence "\<zero> \<otimes> \<zero> \<le> \<one> " unfolding factor_def by auto
...
proof(intro dioid.intro semiring.intro abelian_hemi_group.intro)
  ...
  have "partial_order (canonical_order (add_monoid D\<lparr>carrier := H\<rparr>))"
    using canonical_order_monoid.submonoid_is_canonical_order add.comm_monoid_axioms 
    unfolding abelian_hemi_group_axioms_def canonical_order_monoid_def
              canonical_order_monoid_axioms_def
    by (metis same_canonical_order subdioid.sub_add assms canonically_ordered)
...
lemma real_is_dioid :
"dioid (\<lparr>carrier = {r :: real . r \<ge> 0}, monoid.mult = op *, one = 1, zero = 0, add = op +\<rparr>)"
