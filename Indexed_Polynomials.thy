...
definition (in ring) indexed_const :: "'a \<Rightarrow> ('b multiset \<Rightarrow> 'a)" 
  where "indexed_const k = (\<lambda>m. if m = {#} then k else \<zero>)"

definition (in ring) indexed_pmult :: "('b multiset \<Rightarrow> 'a) \<Rightarrow> 'b \<Rightarrow> ('b multiset \<Rightarrow> 'a)" (infixl "\<Otimes>" 65)
  where "indexed_pmult P i = (\<lambda>m. if i \<in># m then P (m - {# i #}) else \<zero>)"

definition (in ring) indexed_padd :: "_ \<Rightarrow> _ \<Rightarrow> ('b multiset \<Rightarrow> 'a)" (infixl "\<Oplus>" 65)
  where "indexed_padd P Q = (\<lambda>m. (P m) \<oplus> (Q m))"

definition (in ring) indexed_var :: "'b \<Rightarrow> ('b multiset \<Rightarrow> 'a)" ("\<X>\<index>")
  where "indexed_var i = (indexed_const \<one>) \<Otimes> i"

definition (in ring) index_free :: "('b multiset \<Rightarrow> 'a) \<Rightarrow> 'b \<Rightarrow> bool"
  where "index_free P i \<longleftrightarrow> (\<forall>m. i \<in># m \<longrightarrow> P m = \<zero>)"

definition (in ring) carrier_coeff :: "('b multiset \<Rightarrow> 'a) \<Rightarrow> bool"
  where "carrier_coeff P \<longleftrightarrow> (\<forall>m. P m \<in> carrier R)"

inductive_set (in ring) indexed_pset :: "'b set \<Rightarrow> 'a set \<Rightarrow> ('b multiset \<Rightarrow> 'a) set" ("_ [\<X>\<index>]" 80)
  for I and K where
...
fun (in ring) indexed_eval_aux :: "('b multiset \<Rightarrow> 'a) list \<Rightarrow> 'b \<Rightarrow> ('b multiset \<Rightarrow> 'a)"
  where "indexed_eval_aux Ps i = foldr (\<lambda>P Q. (Q \<Otimes> i) \<Oplus> P) Ps (indexed_const \<zero>)"

fun (in ring) indexed_eval :: "('b multiset \<Rightarrow> 'a) list \<Rightarrow> 'b \<Rightarrow> ('b multiset \<Rightarrow> 'a)"
